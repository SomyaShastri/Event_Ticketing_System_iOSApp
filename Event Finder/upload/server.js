            ...
            if ('dates' in events[i] && 'start' in events[i]['dates']) {
                if ('localDate' in events[i]['dates']['start']) {
                    if ('localTime' in events[i]['dates']['start']) {
                        event_details.date = events[i]['dates']['start']['localDate'] + " " + events[i]['dates']['start']['localTime'];
                    } else {
...
async function getArtistDetails(artistName){
    ...
    resp = {};
    // console.log(refinedData[0]?.name.toLowerCase());
    // console.log(artistName.toLowerCase());
    // console.log("check")
    // console.log((refinedData[0]?.name.toLowerCase().trim()).localeCompare(artistName.toLowerCase().trim()));
    if(refinedData.length > 0 && refinedData[0].name){
        if((refinedData[0].name.toLowerCase().trim().localeCompare(artistName.toLowerCase().trim()) === 0)){
...
app.get('/spotify_details', async function (request, res) {
    ...
    for (let artistName of receivedArray) {
        ...
        let data2;
        if(data1.id){
            data2 = await getArtistAlbums(data1.id);
            data1.albums = data2;
